...
class MICCAIBraTS2019(Dataset):
    ...
    """
    def __init__(self, args, mode, dataset_path='../../datasets', classes=4, crop_dim=(128, 128, 128), split_idx=204,
                 samples=10,
                 load=False):
        """
    ...
    def __getitem__(self, index):
        ...
        training_input = torch.FloatTensor(img_t1.copy()).unsqueeze(0), torch.FloatTensor(img_t1ce.copy()).unsqueeze(
            0), torch.FloatTensor(img_t2.copy()).unsqueeze(0), torch.FloatTensor(img_flair.copy()).unsqueeze(
            0), torch.FloatTensor(img_seg.copy())
        # print("training input dim from get:", training_input[0].size())
